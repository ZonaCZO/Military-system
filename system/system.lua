-- === MSOS V1.3 ===
-- [TACTICAL LAUNCHER + PR FOLDER]

local bgCol = colors.gray
local barCol = colors.black
local txtCol = colors.white
local iconCol = colors.blue
local shopCol = colors.green

-- === НАСТРОЙКИ СИСТЕМЫ ===
local sysDir = "sys" -- Системная папка (база данных)
local prDir = "pr"   -- Папка для программ
...
-- Файлы, которые игнорируются при сканировании КОРНЕВОЙ папки
local blacklist = {
    ...
    [".net_config.txt"] = true,
    ["store_data.txt"] = true,
    [sysDir] = true, -- Игнорируем сами папки в меню
    [prDir] = true
}

local mode = "HOME" -- HOME или SHOP
local programs = {}
local shopItems = {
    {name="Worm", code="4YqH58kM", desc="Tunnel Digger"},
    {name="Chat", code="8i7WjZ6b", desc="Simple Rednet Chat"},
...
local function generateEntry(filepath)
    -- Получаем чистое имя файла без пути для красивой иконки
    local name = fs.getName(filepath)
    
    -- Генерируем стиль иконки
    ...
    return {
        name = filepath, -- Полный путь для запуска
        shortName = name, -- Имя для отображения
        label = label,
        color = color
    }
end
...
local function autoScan()
    ...
    -- 1. Сканируем КОРЕНЬ (пользовательские программы)
    ...
    -- 2. Сканируем папку PR (скачанные программы)
...
local function drawHome()
    ...
    for i, prog in ipairs(programs) do
        ...
        -- Подпись (используем shortName для красоты)
        term.setBackgroundColor(bgCol)
        term.setTextColor(txtCol)
        term.setCursorPos(x, y+5)
        local displayName = (prog.shortName or fs.getName(prog.name)):sub(1, 6)
        write(displayName)
...
local function runProgram(filename)
    if not fs.exists(filename) then
        ...
        print(filename)
        sleep(2)
        autoScan() -- Пересканируем, если файл пропал
        return
    end
...
local function downloadProgram(item)
    ...
    if not http then
        ...
        term.setTextColor(colors.red)
        write("Error: HTTP API disabled!")
        sleep(2)
        return
    end
    
    -- Сохраняем в папку PR
    local fileName = fs.combine(prDir, item.name..".lua")
    
    shell.run("pastebin", "get", item.code, fileName)
    
    -- Добавляем в вайт-лист
    local newEntry = generateEntry(fileName)
    
    -- Проверка на дубликаты
    local exists = false
    for _, p in ipairs(programs) do
        if p.name == fileName then exists = true break end
    end
    
    if not exists then
        table.insert(programs, newEntry)
        savePrograms()
    end
...
while true do
        ...
        for _, prog in ipairs(programs) do
            if prog.clickBounds and x >= prog.clickBounds.x1 and x <= prog.clickBounds.x2 
               and y >= prog.clickBounds.y1 and y <= prog.clickBounds.y2 then
                ...
                -- Анимация нажатия
                paintutils.drawFilledBox(prog.clickBounds.x1, prog.clickBounds.y1, 
                                         prog.clickBounds.x2, prog.clickBounds.y2, colors.white)
                sleep(0.1)
    ...
    elseif mode == "SHOP" then
        for _, item in ipairs(shopItems) do
            if item.clickBounds and x >= item.clickBounds.x1 and x <= item.clickBounds.x2
               and y == item.clickBounds.y1 then
