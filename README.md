...
MSOS uses focused text fields instead of a background keyboard-event injector. Press
`F2` while editing a normal text field to switch between `EN`, `RU`, and `UA`. Login IDs,
passwords, network IDs, coordinates, and other machine-readable values stay in English
mode. This prevents a character generated by a hotkey from appearing in the next field.

The soldier PDA is divided into Task, Communications, Alert, and Profile pages. `Tab`
cycles pages, `C` opens communications, `A` opens field alerts, and the top page buttons
can be clicked on devices which provide pointer events. The header shows network state,
input language, and unread communications.
